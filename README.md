...
- Строковые литералы размещаются в памяти в формате Pascal-string, по одному символу в ячейку памяти (без плотной
  упаковки)
- Целочисленные литералы загружаются прямой загрузкой в случае если их значение помещается в 15 бит аргумента
...
- `INT_RQ` -- регистр хранящий флаг запроса прерывания
- `INT_ADDR` -- регистр хранящий адрес подпрограммы обработки прерываний (задаются перед запуском модели)
- `INT_EN` -- регистр хранящие флаг, который определяет разрешены ли прерывания (по умолчанию сброшен)
...
> Так было сделано потому, что до этапа линковки не возможно понять насколько далеко необходимо выполнить переход, а эта
...
> смещение может быть использован переход относительно регистра `JR`, но тогда переход будет занимать больше
...
записывается значение из регистра `PC_BUF`. В конце состояние процессора меняется на `NORMAL`.
...
src/machine/control_unit.py                                261     11    96%   137-138, 164, 166, 351-357
