...
- Выполняет текстовую подстановку ноку всего содержимого указанного файла вместо строки данной
...
4. `INT_EN` -- регистр хранящие флаг, который определяет разрешены ли прерывания (по умолчанию сброшен)
...
- Данные и инструкции сохраняются в виде специальных объектов каждый из которых помимо основной информации хранит также
  адрес
- Строковые переменные, строковые литералы и выделения блоков памяти размещаются в памяти последовательно. При этом по
  мере размещения в таблице символов уточняется адрес соответствующей конструкции, который затем используется для
  определения обращений к памяти
- Численные литералы в начале преобразуется в бинарное представление, после чего загружаются:
    - прямой загрузкой (`addi`) если значения входит в диапазон $[-2^{14}, 2^{14}-1]$
...
- Обработка циклов и условий выполняется при помощи операций условных и безусловных переходов. При этом в случе если
  переход слишком длинный для стандартного безусловного перехода выполняется переход относительно регистра
- Когда списки данных и инструкций сформированы, в конец основной программы добавляется инструкция остановки `halt`,
  после чего происходит линковка:
    - Адреса инструкций расставляются последовательно, при этом основной блок инструкций начинается с нуля, а адрес
      обработчика прерываний можно задать в файле констант [constats](src/constants.py)
    ...
    - Адреса данных проставляются по порядку, при этом адрес начала задаётся в файле констант (так как в начале
      начальные адреса зарезервированы для операций ввода-вывода)
...
иметь разный эффект то возможно несколько реализаций

#### Сигналы Data Path

- `latch_data_address` -- защёлкнуть значение из регистра в адрес памяти данных
- `store_registers` -- защёлкнуть резервный блок регистров
- `restore_registers` -- защёлкнуть основной блок регистров
- `data_memory_store` -- записать значение из регистра в память
...
- `data_memory_load` -- записать значение из памяти в регистр
...
- `perform_alu_operation` -- выполнения операции АЛУ и установка флагов. В зависимости от сигналов мультиплексоров
  возможны следующие конфигурации:
    - `reg` `reg` -- оба операнда - регистры. Результат записывается в регистр
    - `imm` `reg` -- первый операнд - непосредственное значение, после расширения знака, второй операнд - регистр.
      Результат записывается в регистр
    - `uimm` `reg` -- первый операнд - непосредственное значения, сдвинутое влево на 12 бит (
      нужно для операции `lui`), второй операнд - регистр. Результат записывается в регистр
    - `imm` `pc` -- первый операнд - непосредственное значение, после расширения знака, а второй - значения счётчика
      команд, полученное из Control Unit. Результат выставляется на шину `next_pc`, идущую в Control Unit
    - `imm` `reg` -- первый операнд - непосредственное значение, после расширения знака, второй операнд - регистр.
      Результат выставляется на шину `next_pc`, идущую в Control Unit
...
Для обработки прерываний в программной реализации введены состояния процессора:
...
- `latch_pc` -- защёлкнуть значение счётчика команд. В зависимости от конфигурации мультиплексоров
  возможны следующие конфигурации:
    ...
    - `latch_pc_interrupt` -- защёлкнуть значение счётчика команд адресом прерывания
...
src/isa/instructions/instruction.py                         21      1    95%   47
...
src/machine/control_unit.py                                256     11    96%   140-141, 167, 169, 359-365
src/machine/data_path.py                                   100      4    96%   121, 144, 147, 195
...
src/translator/ast_/ast_node_visitor.py                     56     14    75%   56, 59, 62, 65, 68, 71, 74, 77, 80, 83, 86, 89, 92, 95
...
src/translator/code_generator/code_generator.py            103      0   100%
src/translator/code_generator/instruction_producers.py      38      0   100%
...
src/translator/translator.py                                48      9    81%   70-77, 83-87
...
файлах [io](examples/stdlib/io.fs) и [buffer](examples/stdlib/buffer.fs). Эти файлы могут быть подключены вы любую
программу при помощи `#include`, после чего функции из них станут доступны.

### Пример использования модели процессора

```bash
$ ./machine.py out/instructions.bin out/data.bin examples/hello_user_name.txt
DEBUG   machine:simulation    STATE: NORMAL	TICK:   0 PC:   0/0 ADDR:   0 MEM_OUT:   0 T0:   0 T1:   0 T2:   0 T3:   0 SP: 1000 	addi t0, zero, 4
  ...
  DEBUG   machine:simulation    STATE: NORMAL	TICK:   2 PC:   2/0 ADDR:   0 MEM_OUT:   0 T0:   4 T1:   0 T2:   0 T3:   0 SP: 999 	sw t0, sp
  DEBUG   machine:simulation    STATE: NORMAL	TICK:   3 PC:   2/1 ADDR: 999 MEM_OUT:   0 T0:   4 T1:   0 T2:   0 T3:   0 SP: 999 	sw t0, sp
  ...
  DEBUG   machine:simulation    STATE: NORMAL	TICK:   7 PC:   5/0 ADDR: 999 MEM_OUT:   4 T0:   4 T1:   0 T2:   0 T3:   0 SP: 998 	sw t0, sp
  DEBUG   machine:simulation    STATE: NORMAL	TICK:   8 PC:   5/1 ADDR: 998 MEM_OUT:   0 T0:   4 T1:   0 T2:   0 T3:   0 SP: 998 	sw t0, sp
  ...
  DEBUG   machine:simulation    STATE: NORMAL	TICK:  11 PC:   8/0 ADDR: 998 MEM_OUT:   4 T0:   1 T1:   0 T2:   0 T3:   0 SP: 997 	sw t0, sp
  DEBUG   machine:simulation    STATE: NORMAL	TICK:  12 PC:   8/1 ADDR: 997 MEM_OUT:   0 T0:   1 T1:   0 T2:   0 T3:   0 SP: 997 	sw t0, sp
  ...
  DEBUG   machine:simulation    STATE: NORMAL	TICK:  21 PC:  15/0 ADDR: 998 MEM_OUT:   4 T0:   5 T1:   4 T2:   0 T3:   0 SP: 998 	sw t0, sp
  DEBUG   machine:simulation    STATE: NORMAL	TICK:  22 PC:  15/1 ADDR: 998 MEM_OUT:   4 T0:   5 T1:   4 T2:   0 T3:   0 SP: 998 	sw t0, sp
