...
- `perform_alu_operation` -- выполнения операции АЛУ и установка флагов. В зависимости от сигналов мультиплексоров
  возможны следующие конфигурации:
    ...
    - `reg` `uimm` -- первый операнд - регистр, второй операнд - непосредственное значения, сдвинутое влево на 12 бит
    - `pc` `imm` -- первый операнд - значения счётчика команд, полученное из Control Unit, а второй - непосредственное значение, после расширения знака
